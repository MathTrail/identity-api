...
3. Интеграция с Dapr (Identity-Aware Sidecars)Вместо того чтобы каждый микросервис (Task Service, Mentor и т.д.) сам проверял JWT-токен, мы используем Dapr Middleware Pipeline с Ory Oathkeeper.Схема работы:Запрос идет к Sidecar-у Dapr сервиса.Dapr пропускает запрос через Oathkeeper Middleware.Oathkeeper проверяет токен (через Hydra/Kratos) и права (через Keto).Если всё ок, запрос доходит до твоего кода.Настройка Dapr Middleware (identity-middleware.yaml):YAMLapiVersion: dapr.io/v1alpha1
kind: Component
metadata:
...
3. Интеграция через Dapr Middleware (Архитектура Zero Trust)
Когда запрос проходит через Dapr, Ory Oathkeeper выполняет роль «судьи». Он берет сессию из Kratos или токен из Hydra и превращает их в понятные для твоих микросервисов заголовки.
...
Безопасность: ИИ будет генерировать запросы к Dapr, зная, что за авторизацию отвечает внешний Middleware.
